...
wait_for_wrapper()
{
    ...
    if [ "$WAITFORIT_QUIET" -eq 1 ]; then
        timeout ${WAITFORIT_BUSYTIMEFLAG:+$WAITFORIT_BUSYTIMEFLAG} "$WAITFORIT_TIMEOUT" "$0" --quiet --child --host="$WAITFORIT_HOST" --port="$WAITFORIT_PORT" --timeout="$WAITFORIT_TIMEOUT" &
    else
        timeout ${WAITFORIT_BUSYTIMEFLAG:+$WAITFORIT_BUSYTIMEFLAG} "$WAITFORIT_TIMEOUT" "$0" --child --host="$WAITFORIT_HOST" --port="$WAITFORIT_PORT" --timeout="$WAITFORIT_TIMEOUT" &
    fi
    WAITFORIT_PID=$!
...
if [ -n "$WAITFORIT_TIMEOUT_PATH" ] && echo "$WAITFORIT_TIMEOUT_PATH" | grep -q "busybox"; then
    WAITFORIT_ISBUSY=1
