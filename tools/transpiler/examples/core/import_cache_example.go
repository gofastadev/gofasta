...
// demonstrateImportPerformance shows performance characteristics
func demonstrateImportPerformance() {
	fmt.Printf("\n=== Import Performance Comparison ===\n")
	
	packages := []string{"fmt", "os", "io", "time", "context", "sync", "net/http", "encoding/json"}
	iterations := 100
	
	// Test without cache (using fresh importer each time)
	fmt.Printf("Testing without cache benefits...\n")
	start := time.Now()
	
	for i := 0; i < iterations; i++ {
		importer := core.NewCachedImporter(&core.ImportCacheConfig{EnableMetrics: false})
		for _, pkg := range packages {
			importer.Import(pkg)
		}
	}
	
	withoutCacheEfficiency := time.Since(start)
	
	// Test with cache
	fmt.Printf("Testing with cache benefits...\n")
	cachedImporter := core.NewCachedImporter(core.DefaultImportCacheConfig())
	
	start = time.Now()
	
	for i := 0; i < iterations; i++ {
		for _, pkg := range packages {
			cachedImporter.Import(pkg)
		}
	}
	...
	fmt.Printf("  Without cache efficiency: %v\n", withoutCacheEfficiency)
	fmt.Printf("  With cache: %v\n", withCache)
	
	if withoutCacheEfficiency > withCache {
		improvement := float64(withoutCacheEfficiency-withCache) / float64(withoutCacheEfficiency) * 100
		fmt.Printf("  Cache improvement: %.1f%% faster\n", improvement)
	}
	
	// Show final cache statistics
	stats := cachedImporter.GetStatistics()
	fmt.Printf("  Final hit ratio: %.1f%%\n", stats["hit_ratio"])
	fmt.Printf("  Total imports: %d\n", iterations*len(packages))
}