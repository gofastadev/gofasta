...
// demonstrateFileFormatting shows how to format files from disk
func demonstrateFileFormatting() {
	fmt.Printf("\n=== File System Formatting Example ===\n")
	
	// Create temporary directory with test files
	tempDir := "/tmp/format_example"
	os.MkdirAll(tempDir, 0755)
	defer os.RemoveAll(tempDir)
	
	// Create test files with poor formatting
	testFiles := map[string]string{
		"poorly_formatted.go": `package main
import("fmt";"os")
func main(){
if len(os.Args)>1{
...
return fmt.Sprintf("Data{ID:%d,Name:%s}",d.ID,d.Name)
	...
	// Parse and format all files in directory
	parser := core.NewParallelParser(core.DefaultConfig())
	ctx := context.Background()
	
	parseResults, err := parser.ParseDirectory(ctx, tempDir)
	if err != nil {
		log.Printf("Failed to parse directory: %v", err)
		return
	}
	
	// Convert to format-friendly structure
	...
	if err != nil {
		fmt.Printf("Formatting completed with errors: %v\n", err)
	}
	
	// Write formatted files back to disk
	for filename, result := range formatResults {
		if result.Error == nil {
			outputPath := filepath.Join(tempDir, "formatted_"+filepath.Base(filename))
			err := os.WriteFile(outputPath, result.Output, 0644)
			if err != nil {
				log.Printf("Failed to write formatted file: %v", err)
			} else {
				fmt.Printf("✓ Wrote formatted file: %s\n", outputPath)
			}
		}
	}
